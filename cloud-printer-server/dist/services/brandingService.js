...
class BrandingService {
    async generateTheme(brandingData) {
        try {
            const theme = {
                colors: {
            ...
            logger_1.logger.error('Failed to generate theme:', error);
            throw new AppError_1.AppError('Failed to generate theme', 500);
        }
    }
    processColors(colors = {}) {
        ...
        const primaryColor = (0, color_1.default)(processedColors.primary);
        return {
            colors: {
                ...processedColors,
                ...
                textMuted: (0, color_1.default)(processedColors.text).alpha(0.6).rgb().string(),
                border: (0, color_1.default)(processedColors.text).alpha(0.1).rgb().string(),
            },
        };
    }
    processFonts(fonts = {}) {
    ...
    generateTableStyles(theme) {
        return {
            ...
            row: {
                hoverBackgroundColor: (0, color_1.default)(theme.colors.primary).alpha(0.05).rgb().string(),
            },
        };
    }
    sanitizeColor(color) {
