...
export const onboardingController = {
  async createPartner(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await auditService.logEvent(AuditEventType.PARTNER_CREATED, {
        ipAddress: req.ip,
      ...
      await auditService.logEvent(AuditEventType.PARTNER_UPDATED, {
      ...
      logger.error('Partner creation failed:', error);
      next(error);
    }
  },
  ...
  async updateBranding(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const updatedTheme = await onboardingService.updateBranding(
        partnerId,
        brandingData
      );

      await auditService.logEvent(AuditEventType.BRANDING_UPDATED, {
        partnerId,
        ipAddress: req.ip,
        userAgent: req.headers['user-agent'],
        details: {
      ...
      logger.error('Branding update failed:', error);
      next(error);
    }
  },
  ...
  async rotateApiKey(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await auditService.logEvent(AuditEventType.API_KEY_ROTATED, {
        partnerId,
        ipAddress: req.ip,
        userAgent: req.headers['user-agent'],
        details: {
